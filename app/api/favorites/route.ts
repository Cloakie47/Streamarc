...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "add") {
      await supabase.from("favorites").upsert({ user_id, video_id });
      return NextResponse.json({ success: true, favorited: true });
    }

    if (action === "remove") {
      await supabase.from("favorites").delete().eq("user_id", user_id).eq("video_id", video_id);
    ...
    return NextResponse.json({ error: "Favorites operation failed" }, { status: 500 });
  }
}
